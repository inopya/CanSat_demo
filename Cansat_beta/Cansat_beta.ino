...
void setup() 
{
  ...
  for(uint8_t n=0;n<3;n++){
    digitalWrite(uvSensorList[n], LOW);   
  }
...
float obtener_UV_max()
{
  ...
  for(uint8_t j=0;j<3;j++){
    digitalWrite(uvSensorList[j], LOW);             //cortamos la alimentacion a todos los sensores
  }  
  
  for(uint8_t n=0;n<3;n++){

    digitalWrite(uvSensorList[n], HIGH);              //alimentamos los sensores secuencialmente
